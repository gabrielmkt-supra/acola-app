...
export default function PDVPage() {
  ...
  const totalAmount = subtotal + (channel === "ifood" ? deliveryFee : 0);
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const orderPayload = {
        ...
        delivery_fee: channel === "ifood" ? deliveryFee : 0,
      ...
      setClientName("");
      setDeliveryFee(0);
      fetchProducts(); // Atualiza lista com estoque novo
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] overflow-hidden bg-background">
      ...
      {/* Direita: Carrinho */}
      <div className="w-[400px] bg-surface border-l border-primary/5 flex flex-col shadow-2xl">
        <div className="p-8 border-b border-primary/5">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-black text-primary uppercase italic tracking-tighter">Carrinho</h2>
            <span className="bg-secondary text-primary px-3 py-1 rounded-full text-[10px] font-black">{cart.length} ITENS</span>
          </div>
          
          <div className="space-y-4">
            <div className="relative">
               <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-primary/30 text-lg">person</span>
               <input 
                 type="text" 
                 placeholder="Nome do Cliente (Opcional)"
                 value={clientName}
                 onChange={(e) => setClientName(e.target.value)}
                 className="w-full pl-11 pr-4 py-3 bg-background border border-primary/5 rounded-xl text-xs font-bold text-primary outline-none focus:border-secondary/30 transition-all"
               />
            </div>
            ...
            {channel === "ifood" && (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase text-primary/40 ml-1">Taxa iFood (%)</label>
                  <input 
                    type="number"
                    value={ifoodFeePct}
                    onChange={(e) => setIfoodFeePct(Number(e.target.value))}
                    className="w-full px-4 py-3 bg-red-500/5 border border-red-500/10 rounded-xl text-xs font-black text-red-500 outline-none"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black uppercase text-primary/40 ml-1">Entrega (R$)</label>
                  <input 
                    type="number"
                    value={deliveryFee}
                    onChange={(e) => setDeliveryFee(Number(e.target.value))}
                    className="w-full px-4 py-3 bg-background border border-primary/5 rounded-xl text-xs font-black text-primary outline-none"
                  />
                </div>
              </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-primary/20 gap-4 opacity-40">
              ...
              <p className="text-[10px] font-black uppercase tracking-widest">O carrinho está vazio</p>
            </div>
          ) : (
            <AnimatePresence>
              {cart.map((item) => (
                <motion.div 
                  ...
                  exit={{ opacity: 0, x: -20 }}
                  className="flex items-center gap-4 bg-background/50 p-4 rounded-2xl border border-primary/5 group"
                >
                  <div className="flex-1">
                    <h4 className="text-[11px] font-black text-primary uppercase tracking-tight mb-1">{item.name}</h4>
                    <p className="text-[10px] font-bold text-secondary">R$ {formatUnitCost(item.price)}</p>
                  </div>
                  
                  <div className="flex items-center gap-3">
                    <div className="flex items-center bg-background rounded-xl border border-primary/5 p-1">
                      <button onClick={() => updateQty(item.id, -1)} className="w-6 h-6 flex items-center justify-center hover:bg-primary/5 rounded-lg text-primary/40 hover:text-primary transition-all">
                    ...
                    <button onClick={() => removeFromCart(item.id)} className="w-8 h-8 flex items-center justify-center rounded-xl text-error/30 hover:text-error hover:bg-error/10 transition-all">
        ...
        <div className="p-8 bg-background/50 border-t border-primary/5 space-y-6">
          <div className="space-y-2">
            ...
            {channel === "ifood" && (
              <>
                <div className="flex justify-between text-red-500/60 text-[10px] font-bold uppercase tracking-widest">
            ...
            <div className="flex justify-between items-end">
               <div>
                  <p className="text-[10px] font-black text-primary/40 uppercase tracking-[0.2em] mb-1">Total a Pagar</p>
                  <p className="text-4xl font-black text-primary italic tracking-tighter">R$ {formatUnitCost(totalAmount)}</p>
               </div>
               {channel === "ifood" && (
                 <div className="text-right">
                    <p className="text-[9px] font-black text-success uppercase tracking-widest mb-1">Lucro Líquido</p>
                    <p className="text-xl font-black text-success tracking-tight">R$ {formatUnitCost(netAmount)}</p>
                 </div>
               )}
            </div>
          </div>
          ...
          <button 
            ...
            className={cn(
              "w-full py-6 rounded-[24px] text-xs font-black uppercase tracking-[0.3em] transition-all flex items-center justify-center gap-3 shadow-xl",
